...
object RedisData {
    ...
    /**
     * 从redis获取这个玩家的区块信息,0，1，2分别代表主世界，地狱和末地
     */
    fun getChunks(uuidString: String,worldType: Byte):Set<Pair<Int,Int>>?{
        ...
        return stringChunks.split("|").toMutableList().dropLast(1).map { it.split(",")[0].toInt() to it.split(",")[1].toInt() }.toSet()
    }
    /**
     * 把这个玩家世界的区块信息存入redis,0，1，2分别代表主世界，地狱和末地
     */
