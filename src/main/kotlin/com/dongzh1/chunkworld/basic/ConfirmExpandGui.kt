...
class ConfirmExpandGui(private val p: Player,private val chunk: Chunk) {
    ...
    private fun deduct():Boolean{
        ...
        if (chunk.world.environment == World.Environment.NORMAL){
            if (p.inventory.hasItem(amount = chunkLevel, matcher = {type == Material.PAPER && itemMeta.customModelData == 300008 && hasLore("§c已绑定${p.name}")})){
                p.inventory.takeItem(amount = chunkLevel, matcher = {type == Material.PAPER && itemMeta.customModelData == 300008 && hasLore("§c已绑定${p.name}")})
                return true
            }else{
                return false
            }
        }else{
            if (p.inventory.hasItem(amount = chunkLevel, matcher = {type == Material.PAPER && itemMeta.customModelData == 300009 && hasLore("§c已绑定${p.name}")})){
                p.inventory.takeItem(amount = chunkLevel, matcher = {type == Material.PAPER && itemMeta.customModelData == 300009 && hasLore("§c已绑定${p.name}")})
                return true
            }else{
                return false
            }
