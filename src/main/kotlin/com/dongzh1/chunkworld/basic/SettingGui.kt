...
class SettingGui(private val p: Player,private val banPage:Int) {
    fun build() {
        ...
        basic.rows(6)
        val playerDao = SingleListener.getPlayerDaoMap(p.uniqueId)!!
        ...
        val trustList = SingleListener.getTrustMap(p)!!.toList()
        val banList :List<UUID> = if (banPage == 1) SingleListener.getBanMap(p)!!.toList().take(banPage*7-1).toList()
        else SingleListener.getBanMap(p)!!.toList().drop((banPage-1)*7-1).take(banPage*7-1).toList()
        ...
        basic.onClick(10) {
            if (p.location.world.name == ChunkWorld.inst.config.getString("World")!!+"/${p.uniqueId}"){
                ...
                //重新获取数据
                    SingleListener.setPlayerDaoMap(p.name,ChunkWorld.db.playerGet(p.name)!!)
                }
                p.closeInventory()
        ...
        basic.onClick(listOf(19,20,21,22,23,24,25,26)) {
            ...
            //解除信任关系
            val trustMap = SingleListener.getTrustMap(p)!!.toMutableSet()
            trustMap.remove(uuid)
            SingleListener.setTrustMap(p,trustMap)
            val id1 = playerDao.id
            val targetPlayer = Bukkit.getPlayer(uuid)
            if (targetPlayer != null){
                targetPlayer.sendMessage("§c${p.name} 已和你解除信任关系")
                val trustDao1 = SingleListener.getTrustMap(targetPlayer)!!.toMutableSet()
                trustDao1.remove(p.uniqueId)
                SingleListener.setTrustMap(targetPlayer, trustDao1)
            }
        ...
        basic.onClick(listOf(28,29,30,31,32,33,34)) {
            ...
            p.closeInventory()
            if (targetPlayer != null){
                SingleListener.setCommand("${p.name} unban ${targetPlayer.name}")
                TellrawJson()
